        ...
        {selectedStock && stockQuote && viewMode === 'spacious' && (
          <div className={`space-y-6 ${stockAnimating ? 'stock-slide-in' : 'fade-in'}`} data-testid="stock-details-container">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Row 1: Key Stats and Analysis */}
              <Card className="premium-card gold-gradient-border" data-testid="stats-panel">
                <CardHeader className="pb-4">
                  <CardTitle className="text-base text-white section-title-gold" style={{ fontFamily: 'Outfit, sans-serif' }}>
                    Key Stats
                  </CardTitle>
                ...
                <CardContent>
                  <div className="grid grid-cols-3 gap-4">
                    {stockQuote.day_open && (
                      <div className="stat-item" data-testid="day-open-stat">
                        <div className="text-sm text-gray-400">Day Open</div>
                        <div className="text-base font-semibold mt-1 text-white mono-numbers">
                          {getCurrencySymbol(stockQuote.ticker, stockQuote.currency)}{stockQuote.day_open.toFixed(2)}
                        </div>
                      </div>
                    )}
                    
                    {stockQuote.day_high && (
                      <div className="stat-item" data-testid="day-high-stat">
                        <div className="text-sm text-gray-400">Day High</div>
                        <div className="text-base font-semibold mt-1 text-white mono-numbers">
                          {getCurrencySymbol(stockQuote.ticker, stockQuote.currency)}{stockQuote.day_high.toFixed(2)}
                        </div>
                      </div>
                    )}
                    
                    {stockQuote.day_low && (
                      <div className="stat-item" data-testid="day-low-stat">
                        <div className="text-sm text-gray-400">Day Low</div>
                        <div className="text-base font-semibold mt-1 text-white mono-numbers">
                          {getCurrencySymbol(stockQuote.ticker, stockQuote.currency)}{stockQuote.day_low.toFixed(2)}
                        </div>
                      </div>
                    )}
                    
                    {stockQuote.market_cap && (
                      <div className="stat-item" data-testid="market-cap-stat">
                        <div className="text-sm text-gray-400">Market Cap</div>
                        <div className="text-base font-semibold mt-1 text-white mono-numbers">
                          ${(stockQuote.market_cap / 1e9).toFixed(2)}B
                        </div>
                      </div>
                    )}
                  
                  {stockQuote.pe_ratio && (
                    <div className="stat-item" data-testid="pe-ratio-stat">
                      <div className="text-sm text-gray-400">P/E Ratio</div>
                      <div className="text-base font-semibold mt-1 text-white mono-numbers">
                        {stockQuote.pe_ratio.toFixed(2)}
                      </div>
                    </div>
                  )}
                  
                  {stockQuote.dividend_yield !== null && stockQuote.dividend_yield !== undefined && (
                    <div className="stat-item" data-testid="dividend-stat">
                      <div className="text-sm text-gray-400">Dividend</div>
                      <div className="text-base font-semibold mt-1 text-white mono-numbers">
                        {stockQuote.dividend_yield.toFixed(2)}%
                      </div>
                    </div>
                  )}
                  
                  {stockQuote.high_52week && (
                    <div className="stat-item" data-testid="52week-high-stat">
                      <div className="text-sm text-gray-400">52 Week High</div>
                      <div className="text-base font-semibold mt-1 text-white mono-numbers">
                        {getCurrencySymbol(stockQuote.ticker, stockQuote.currency)}{stockQuote.high_52week.toFixed(2)}
                      </div>
                    </div>
                  )}
                  
                  {stockQuote.low_52week && (
                    <div className="stat-item" data-testid="52week-low-stat">
                      <div className="text-sm text-gray-400">52 Week Low</div>
                      <div className="text-base font-semibold mt-1 text-white mono-numbers">
                        {getCurrencySymbol(stockQuote.ticker, stockQuote.currency)}{stockQuote.low_52week.toFixed(2)}
                      </div>
                    </div>
                  )}
                  
                  {stockQuote.volume && (
                    <div className="stat-item" data-testid="volume-stat">
                      <div className="text-sm text-gray-400">Volume</div>
                      <div className="text-base font-semibold mt-1 text-white mono-numbers">
                        {(stockQuote.volume / 1e6).toFixed(2)}M
                      </div>
                    </div>
                  )}
                </div>
