...
import CapitalDeploymentsPreview from './CapitalDeploymentsPreview';
...
const HomePage = () => {
  ...
  // Per-ticker refresh state for the inline details blocks on non-last stacked stocks
  const [refreshingTickers, setRefreshingTickers] = useState(new Set());
  // Expanded state for the per-stock inline accordion (older stacked stocks).
  // The last stacked stock is always expanded; this set tracks the others the user opened.
  ...
  // Refresh health report for an arbitrary ticker (used by inline detail blocks
  // on older stacked stocks). Tracks per-ticker refreshing state so multiple
  // can refresh in parallel without colliding.
  const refreshHealthReportForTicker = async (ticker) => {
    if (!ticker) return;
    ...
    // Optimistic: mark this stock's healthReport as null so loading state shows
    setStackedStocks(prev => prev.map(s =>
      s.ticker === ticker ? { ...s, healthReport: null } : s
    ));

    try {
      const timeoutPromise = (promise, ms = 20000) => Promise.race([
        promise,
        new Promise((_, reject) => setTimeout(() => reject(new Error('Request timeout')), ms))
      ]);
      const response = await timeoutPromise(axios.get(`${API}/stocks/${ticker}/health-report`));
      const healthData = response.data;
      const validHealth = (healthData && healthData.quarters && healthData.quarters.length > 0)
        ? healthData
        : { _failed: true, message: 'No financial data available for this stock' };

      setStackedStocks(prev => prev.map(s =>
        s.ticker === ticker ? { ...s, healthReport: validHealth } : s
      ));
      if (ticker === selectedStock) setHealthReport(validHealth);

      if (validHealth._failed) {
        toast.error('Could not load AI analysis - no data available');
      } else {
        toast.success(`AI Analysis refreshed for ${ticker}`);
      }
    } catch (error) {
      console.error('Error refreshing health report for', ticker, error);
      const failed = { _failed: true, message: error.message || 'Failed to load health report' };
      setStackedStocks(prev => prev.map(s =>
        s.ticker === ticker ? { ...s, healthReport: failed } : s
      ));
      if (ticker === selectedStock) setHealthReport(failed);
      toast.error('Failed to refresh AI analysis. Please try again.');
    } finally {
      setRefreshingTickers(prev => {
        const next = new Set(prev);
  ...
  const toggleExpandedTicker = (ticker) => {
        ...
        {viewMode === 'spacious' && stackedStocks.length > 0 && (
          <div className="space-y-4" data-testid="stacked-stocks-container">
            ...
            {stackedStocks.map((stock, index) => {
              const isLast = index === stackedStocks.length - 1;
              const isExpanded = isLast || expandedTickers.has(stock.ticker);
              return (
              <div 
                key={stock.ticker} 
              ...
              >
                ...
                {/* Inline per-stock details. Last stock is always expanded;
                    older stocks render a "Show full analysis" toggle. */}
                {!isLast && (
                  <button
                    type="button"
                    onClick={() => toggleExpandedTicker(stock.ticker)}
                    data-testid={`toggle-details-${stock.ticker}`}
                    className="mt-3 w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.02)] hover:bg-[rgba(217,70,239,0.06)] hover:border-[#d946ef]/40 text-xs sm:text-sm text-gray-300 hover:text-white transition-all duration-200 group"
                  >
                    <ChevronDown
                      className={`w-4 h-4 text-[#d946ef] transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`}
                    />
                    <span>
                      {isExpanded ? 'Hide' : 'Show'} full analysis
                      <span className="hidden sm:inline text-gray-500"> · Financials · News · AI · Intelligence</span>
                    </span>
                  </button>
                )}
                ...
                {isExpanded && (
                  <StockDetailsBlock
                    stock={stock}
                    onRefreshHealth={refreshHealthReportForTicker}
  ...
  return (
    <>
      <Routes>
        ...
        <Route path="/preview/capital-deployments" element={<CapitalDeploymentsPreview />} />
        <Route path="/*" element={
          <>
            <AccessGate>
              <Routes>
